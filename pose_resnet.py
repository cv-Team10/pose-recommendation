...
class CustomBlock(nn.Module):
    def __init__(self, extra):
        ...
        self.fl = nn.Flatten(2,-1)
        self.regression = nn.Linear(extra.HEATMAP_SIZE[0]*extra.HEATMAP_SIZE[1], 18)
        self.layer1 = nn.Linear(18,18)
        self.layer2 = nn.Linear(18,18)
        self.layer3 = nn.Linear(18,3)
    ...
    def forward(self, x):
        x = self.fl(x)
        x = self.relu(self.regression(x))
        x = self.relu(self.layer1(x))
        x = self.relu(self.layer2(x))
        x = self.layer3(x)
        reg, cls = torch.split(x, [2, 1], dim=-1)
...
#    def __init__(self, block, layers, cfg, **kwargs):
    def __init__(self, block, layers, **kwargs):
        ...
        self.custom_layer = CustomBlock(extra) #TAK
        ...
        if os.path.isfile(pretrained):
            ...
            for m in self.final_layer.modules():
                if isinstance(m, nn.Conv2d):
                    ...
                    nn.init.normal_(m.weight, std=0.001)
                    nn.init.constant_(m.bias, 0)
            #linear layer는 자동으로 xavier init이 된다고 한다
